...
interface ExemploAtendimento {
  id: string;
  texto: string;
}

export default function AtendimentoForm() {
  const [exemplos, setExemplos] = useState<ExemploAtendimento[]>([]);
  ...
  useEffect(() => {
    ...
    if (!db) return;
    setLoading(true);
    db.collection('ia_training_atendimento')
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!db) return;
    try {
      const docRef = await db.collection('ia_training_atendimento').add({ texto: novoExemplo, createdAt: new Date() });
  ...
  const handleRemove = async (id: string) => {
    ...
    if (!db) return;
    try {
      await db.collection('ia_training_atendimento').doc(id).delete();
  ...
  const handleEditSave = async () => {
    ...
    if (!db) return;
    try {
      await db.collection('ia_training_atendimento').doc(editId).update({ texto: editTexto });
    ...
    <div>
      <form onSubmit={handleAdd} className="flex gap-2 mb-6">
        <input
          ...
          placeholder="Exemplo de atendimento..."
          value={novoExemplo}
