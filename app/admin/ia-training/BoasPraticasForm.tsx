...
interface ExemploBoasPraticas {
  id: string;
  texto: string;
}

export default function BoasPraticasForm() {
  const [exemplos, setExemplos] = useState<ExemploBoasPraticas[]>([]);
  ...
  useEffect(() => {
    ...
    if (!db) return;
    setLoading(true);
    db.collection('ia_training_boaspraticas')
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!db) return;
    try {
      const docRef = await db.collection('ia_training_boaspraticas').add({ texto: novoExemplo, createdAt: new Date() });
  ...
  const handleRemove = async (id: string) => {
    ...
    if (!db) return;
    try {
      await db.collection('ia_training_boaspraticas').doc(id).delete();
  ...
  const handleEditSave = async () => {
    ...
    if (!db) return;
    try {
      await db.collection('ia_training_boaspraticas').doc(editId).update({ texto: editTexto });
    ...
    <div>
      <form onSubmit={handleAdd} className="flex gap-2 mb-6">
        <input
          ...
          placeholder="Exemplo de boas práticas..."
          value={novoExemplo}
